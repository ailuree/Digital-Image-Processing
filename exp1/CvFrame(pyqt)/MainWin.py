...
class UniformSampling(ProcessingStrategy):
    ...
    def apply(self, img):
        # 对图像均匀采样处理
        # 确保步长不超过图像尺寸
        interval = min(self.interval, img.shape[0], img.shape[1])  # 图像的行数和列数（高度和宽度）
        # 采样处理  （使用NumPy的切片）
        sampled_img = img[::interval, ::interval]  # 行、列方向上按照间隔interval采样
        # OpenCv的resize函数插值填充（最近邻插值法），恢复原始图像尺寸
...
class NonUniformSampling_opencv(ProcessingStrategy):
    ...
    def apply(self, img):
        ...
        # 获取采样后的图像尺寸
        sampled_height, sampled_width = sampled_img.shape[:2]   # 获取前两个维度的值 高宽   
        original_height, original_width = img.shape[:2]
        
        # 创建一个空的图像矩阵用于存储插值结果
        resized_img = np.zeros((original_height, original_width, img.shape[2]), dtype=img.dtype) if len(img.shape) == 3 else np.zeros((original_height, original_width), dtype=img.dtype)
        
        # 双线性插值  向后映射法（像素填充）
        for i in range(original_height):
            for j in range(original_width):
                # 计算在采样图像中的位置（也就是原图像中的位置）
                x = i / (original_height - 1) * (sampled_height - 1)
                y = j / (original_width - 1) * (sampled_width - 1)
                
                # 获取周围的四个像素点
                x1, y1 = int(x), int(y)
                x2, y2 = min(x1 + 1, sampled_height - 1), min(y1 + 1, sampled_width - 1)
                ...
                if len(img.shape) == 3:  # 彩色图像
                    for c in range(img.shape[2]):
                        resized_img[i, j, c] = (1 - a) * (1 - b) * sampled_img[x1, y1, c] + a * (1 - b) * sampled_img[x2, y1, c] + (1 - a) * b * sampled_img[x1, y2, c] + a * b * sampled_img[x2, y2, c]
                else:  # 灰度图像
                    resized_img[i, j] = (1 - a) * (1 - b) * sampled_img[x1, y1] + a * (1 - b) * sampled_img[x2, y1] + (1 - a) * b * sampled_img[x1, y2] + a * b * sampled_img[x2, y2]
